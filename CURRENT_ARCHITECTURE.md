...
## 10. 2026-03-22 翻新增量（已落地）
- Heartbeat 频率动态化（降噪）：
  - 忙时 30 分钟：`queue_depth >= 1` 或 `queue_lag_p95 >= 15000ms` 或 mode in `stressed,degraded`
  - 空闲 60 分钟：其余场景
  - 关键配置：`DAYTIME_HEARTBEAT_INTERVAL_BUSY_MIN=30`、`DAYTIME_HEARTBEAT_INTERVAL_IDLE_MIN=60`
- 收尾后静默：
  - `END_HOUR` 仅在整点发送一次 closing 文案
  - 当日剩余时段不再继续心跳问候，避免“晚安后仍刷屏”
- 回执语气统一：
  - Feishu 最终回执改为“陪伴感 + 证据路径”语气
  - 字段结构保持兼容，不影响验收和追踪
- 配置归档：
  - 新增 `config/rawcli/event-phrases.yaml`，将事件话术池纳入 Beatless 仓库版本化管理

## 11. 2026-03-23 配置同步（已落地）
- 新增 5 个 Core Agent 的 `AGENTS.md` 版本化落盘：
  - `souls/001-kouka/AGENTS.md`
  - `souls/002-snowdrop/AGENTS.md`
  - `souls/003-satonus/AGENTS.md`
  - `souls/004-methode/AGENTS.md`
  - `souls/005-lacia/AGENTS.md`
- `SOUL.md` 与 runtime 工作区重新对齐（含 Lacia ACK 规则修订）。
- 新增运行配置快照（去密钥）：
  - `config/runtime/openclaw.runtime-overlay.redacted.json`
  - 包含：`commands`、`channels.feishu`、`skills`、`agents.defaults.memorySearch`
